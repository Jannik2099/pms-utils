...
public:
    ...
    BOOST_DESCRIBE_CLASS(Content, (), (data), (), (ostream_impl));
};
...
public:
    [[nodiscard]] explicit Entry(std::filesystem::path path);
...
public:
    [[nodiscard]] explicit Vdb(std::filesystem::path path);
...
private:
    [[nodiscard]] Iterator(std::filesystem::directory_iterator iter, std::optional<Category> category);

    std::filesystem::directory_iterator it;
    std::optional<value_type> category;
...
static_assert(std::forward_iterator<Vdb::Iterator>);
...
private:
    [[nodiscard]] Iterator(std::filesystem::directory_iterator iter, std::optional<value_type> entry);
...
static_assert(std::forward_iterator<Category::Iterator>);
...
template <> struct is_owning_iterator<vdb::Vdb::Iterator> {
    constexpr static bool value = true;
};
template <> struct is_owning_iterator<vdb::Category::Iterator> {
    constexpr static bool value = true;
};
